...
export function calculateDashboardStats(projects) {
  ...
  return {
    ...
    // prioritize totalCommits, fall back to commits array length
    totalCommits: projects.reduce((sum, project) => {
      const commitCount =
        project.totalCommits ||
        (Array.isArray(project.commits)
...
export function getCommitActivityData(projects) {
  ...
  projects.forEach((project) => {
    // Use totalCommits if available, otherwise fall back to commits array length
    const commitCount =
      project.totalCommits ||
      (Array.isArray(project.commits)
        ? project.commits.length
    ...
    if (commitCount > 0) {
      const date = new Date(
        project.lastCommitDate || project.updatedAt || project.createdAt
